...
class DataRepository private constructor(private val apiService: ApiService) {
    ...
    suspend fun getComments(postId: Int): Response<List<CommentResponseItem>> = apiService.getComments(postId)
    suspend fun getAlbums(userId: Int): Response<List<AlbumResponseItem>> = apiService.getAlbums(userId)
    suspend fun getPhotos(photoId: Int): Response<List<PhotoResponseItem>> = apiService.getPhotos(photoId)
